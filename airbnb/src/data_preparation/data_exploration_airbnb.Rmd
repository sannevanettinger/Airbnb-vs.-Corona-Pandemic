...
# Count the different neighbourhoods
...
## Conclusion: (1) the variable neighbourhoods consists of too many categories to run a clear plot, we need to make categories out of them (I found out how, but it has way too much different neighbourhoods to do it manually). (2) The variable room_type clearly does not show many hotel rooms and shared rooms across from the other two types. (3) Calculated host listings has some outliers around 600 and around 900. (4) Review rating scores show a few outliers which are very low. We need to find out if these have a valuable reason. (5) Finally, price needs to be looked at more closely because the outlier-parts are in the middle - in this case - but not necessarily present. 
```
-----
Note Jurg: I do not think we should try to plot the different neigbourhoods, maybe we can do just a count of the different neighbourhoods. Room type, however, is interesting to see. 
-----
```