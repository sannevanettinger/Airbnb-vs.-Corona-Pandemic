...
data_uk = do.call('rbind', listings_uk)
...
write.csv(data_uk, 'datauk.csv', row.names=F)
...
```{r}
# To determine the maximum per year/per quarter, make new variable rows 
data_uk$Max_qrt1 <- max(listings_london_qrt1$review_scores_rating, na.rm=TRUE)
data_uk$Max_qrt2 <- max(listings_london_qrt2$review_scores_rating, na.rm=TRUE)
data_uk$Max_qrt3 <- max(listings_london_qrt3$review_scores_rating, na.rm=TRUE)
data_uk$Max_qrt4 <- max(listings_london_qrt4$review_scores_rating, na.rm=TRUE)
...
## I also wanted to make an alternative - using else if - when the rating is between 0 and 10 (instead of 0-5 and 0-100), but than you also take the "bigger than 5" so I did not find a way to get these separated. 
### The dataframes mentioned are maybe not the right ones, depending on the previous steps. Revise. 

old_value=data_uk$Max_qrt1
ifelse(data_uk$Max_qrt1>5, old_value/20, old_value)
df %>% mutate(data_uk$new_col = ifelse(data_uk$Max_qrt1>5, old_value/20, old_value))
