...
```{r Summary statistics on the complete dataset}
summary(data_airbnb_uk)
```

```{r Summary statistics variable: city}
dfSummary(data_airbnb_uk$city)
```

```{r Summary statistics variable: neighbourhood_cleansed (1)}
# Number of neighbourhoods in total
unique(data_airbnb_uk$neighbourhood_cleansed)
```

```{r Summary statistics variable: neighbourhood_cleansed (2)}
# Count combinations of cities and neighbourhoods
data_airbnb_uk %>% 
  count(neighbourhood_cleansed, city, sort = TRUE)
```

```{r Summary statistics variable: room_type (1)}
dfSummary(data_airbnb_uk$room_type)
```

```{r Summary statistics variable: room_type (2)}
...
```{r Summary statistics variable: room_type (3)}
...
```{r Summary statistics London: neighbourhood_cleansed & room_type}
...
```{r Summary statistics Great Manchester: neighbourhood_cleansed & room_type}
...
```{r Summary statistics Bristol: neighbourhood_cleansed & room_type}
...
```{r Detect origin missing values using host_is_superhost}
data_airbnb_uk %>%
...
```{r Detect origin missing values using host_since}
...
```{r Detecting outliers: price (1)}
boxplot(data_airbnb_uk_cleaned$price,
  ylab = "price")
```

```{r Detecting outliers: price (2)}
...
```{r Summary statistics variable: room_type}
dfSummary(data_airbnb_uk_cleaned$room_type)
```

```{r Summary statistics variable: room_type & city}
# Plot room_type per city
...
```{r Summary statistics variable: neighbourhood_name }
unique(data_airbnb_uk_cleaned$neighbourhood_name)
```

```{r Summary statistics variable: quarter}
...
```{r Write the cleaned data to a CSV file}
# Exporting the cleaned data to a CSV file
dir.create("../../gen/analysis/input/")
write.csv(data_airbnb_uk_cleaned, '../../gen/analysis/input/data_airbnb_uk_cleaned.csv', row.names = F)
```
